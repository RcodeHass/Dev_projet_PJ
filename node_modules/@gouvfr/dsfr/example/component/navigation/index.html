<!doctype html>
<html lang="fr" data-fr-theme>

<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
    <meta name="format-detection" content="telephone=no,date=no,address=no,email=no,url=no">
    <title>Navigation principale - Système de design</title>
    <meta name="theme-color" content="#000091"><!-- Défini la couleur de thème du navigateur (Safari/Android) -->
    <link rel="apple-touch-icon" href="../../../dist/favicon/apple-touch-icon.png"><!-- 180×180 -->
    <link rel="icon" href="../../../dist/favicon/favicon.svg" type="image/svg+xml">
    <!-- si favicon.ico peut etre placé à la racine du serveur, retirer la ligne ci-dessous -->
    <link rel="shortcut icon" href="../../../dist/favicon/favicon.ico" type="image/x-icon"><!-- 32×32 -->
    <link rel="manifest" href="../../../dist/favicon/manifest.webmanifest" crossorigin="use-credentials">
    <!-- Modifier les chemins relatifs des favicons en fonction de la structure du projet -->
    <!-- Dans le fichier manifest.webmanifest aussi, modifier les chemins vers les images -->
    <link href="../../../dist/core/core.main.css" rel="stylesheet">
    <link href="../../../dist/component/link/link.main.css" rel="stylesheet">
    <link href="../../../dist/component/button/button.main.css" rel="stylesheet">
    <link href="../../../dist/scheme/scheme.css" rel="stylesheet">
    <link href="../../../dist/component/accordion/accordion.main.css" rel="stylesheet">
    <link href="../../../dist/component/form/form.main.css" rel="stylesheet">
    <link href="../../../dist/component/navigation/navigation.main.css" rel="stylesheet">
    <link href="../../../dist/component/radio/radio.main.css" rel="stylesheet">
    <link href="../../../dist/component/modal/modal.main.css" rel="stylesheet">
    <link href="../../../dist/utility/utility.main.css" rel="stylesheet">
    <link href="../../../dist/core/core.legacy.css" rel="stylesheet">
    <link href="../../../dist/component/link/link.legacy.css" rel="stylesheet">
    <link href="../../../dist/component/button/button.legacy.css" rel="stylesheet">
    <link href="../../../dist/component/accordion/accordion.legacy.css" rel="stylesheet">
    <link href="../../../dist/component/form/form.legacy.css" rel="stylesheet">
    <link href="../../../dist/component/navigation/navigation.legacy.css" rel="stylesheet">
    <link href="../../../dist/component/radio/radio.legacy.css" rel="stylesheet">
    <link href="../../../dist/component/modal/modal.legacy.css" rel="stylesheet">
    <link href="../../../dist/utility/utility.legacy.css" rel="stylesheet">
    <link href="../../../dist/core/core.print.css" rel="stylesheet">
    <link href="../../../dist/component/link/link.print.css" rel="stylesheet">
    <link href="../../../dist/component/button/button.print.css" rel="stylesheet">
    <link href="../../../dist/component/accordion/accordion.print.css" rel="stylesheet">
    <link href="../../../dist/component/form/form.print.css" rel="stylesheet">
    <link href="../../../dist/component/navigation/navigation.print.css" rel="stylesheet">
    <link href="../../../dist/component/radio/radio.print.css" rel="stylesheet">
    <link href="../../../dist/component/modal/modal.print.css" rel="stylesheet">
    <link href="../../../dist/utility/utility.print.css" rel="stylesheet">
    <style>
        code[class*="language-"],
        pre[class*="language-"] {
            white-space: pre-wrap;
        }

        .example-controls {
            z-index: 100000;
            position: fixed;
            top: 0;
            left: 0;
            right: 0;
            box-shadow: 0 8px 16px 0 rgba(0, 0, 0, 0.1), 0 8px 16px -16px rgba(0, 0, 0, 0.32);
        }

        .example-controls>button {
            position: absolute;
            right: 0;
            top: 0;
        }

        .example-controls__content {
            padding: 16px;
            background-color: var(--background-elevated-white);
        }

        .example-controls .fr-collapse--expanded~[aria-expanded] {
            display: none;
        }

        .example-controls .fr-collapse:not(.fr-collapse--expanded)~button:not([aria-expanded]) {
            display: none;
        }

        .example-controls [class*="-form-group"] {
            margin-bottom: 0;
        }

        .fr-display-button {
            position: fixed;
            bottom: 3.5rem;
            right: 2rem;
        }

        @media screen and (-ms-high-contrast: active),
        (-ms-high-contrast: none) {

            /* IE10+ specific styles go here */
            .example-controls {
                display: none;
            }
        }

    </style>
    <script>
    window['dsfr'] = {
      // verbose: true,
      // production: true,
      // mode: 'manual',
    };
    
  </script>
    <link href="https://cdnjs.cloudflare.com/ajax/libs/prism/1.20.0/themes/prism.min.css" rel="stylesheet" />
    <link href="https://cdnjs.cloudflare.com/ajax/libs/prism/1.20.0/plugins/toolbar/prism-toolbar.min.css" rel="stylesheet" />
</head>

<body id="top" class="fr-mb-12w">
    <script type="module">
    const e="system",t="dark",c="dark",o="data-fr-theme",a="data-fr-scheme",r=`:root[${o}], :root[${a}]`,m=()=>{document.documentElement.setAttribute(o,c),document.documentElement.style.colorScheme="dark"},n=()=>{window.matchMedia("(prefers-color-scheme: dark)").matches&&m()};(()=>{if(document.documentElement.matches(r)){const c=(()=>{try{return"localStorage"in window&&null!==window.localStorage}catch(e){return!1}})()?localStorage.getItem("scheme"):"",o=document.documentElement.getAttribute(a);switch(!0){case c===t:m();break;case c===e:n();break;case o===t:m();break;case o===e:n()}}})();

  </script>
    <span id="[url - à modifier]"></span>
    <div class="fr-container fr-mt-2v fr-mb-8w">
        <p class="fr-text--sm" style="float:right">DSFR v1.13.0</p>
        <p>
            <a class="fr-icon-arrow-left-line fr-link--icon-left fr-link" href="../">Retour</a>
        </p>
        <h1>Navigation principale (navigation)</h1>
        <p>La navigation principale est l'élément central de la navigation au sein du site, il oriente l’utilisateur à travers les grandes sections du site et sur éventuellement plusieurs niveaux de profondeur.</p>
        <a href="https://www.systeme-de-design.gouv.fr/elements-d-interface/composants/navigation-principale" rel="noopener external" target="_blank" title="${getBlankTitle('Documentation')}">Documentation</a>
    </div>
    <div class="fr-mb-12w">
        <div class="fr-container">
            <h4 class="fr-h4">Navigation Principale</h4>
        </div>
        <div class="fr-mb-6v">
            <div style="position:relative">
                <div class="fr-container">
                    <nav class="fr-nav" id="navigation-6110" role="navigation" aria-label="Menu principal">
                        <ul class="fr-nav__list">
                            <li class="fr-nav__item">
                                <button id="menu-6111" aria-expanded="false" aria-controls="collapse-6112" type="menu" class="fr-nav__btn">Entrée menu</button>
                                <div class="fr-collapse fr-menu" id="collapse-6112">
                                    <ul class="fr-menu__list">
                                        <li>
                                            <a id="nav-6113" href="#" class="fr-nav__link">Lien de navigation nav-6113</a>
                                        </li>
                                        <li>
                                            <a id="nav-6114" href="#" class="fr-nav__link">Lien de navigation nav-6114</a>
                                        </li>
                                        <li>
                                            <a id="nav-6115" href="#" class="fr-nav__link">Lien de navigation nav-6115</a>
                                        </li>
                                        <li>
                                            <a id="nav-6116" href="#" class="fr-nav__link">Lien de navigation nav-6116</a>
                                        </li>
                                        <li>
                                            <a id="nav-6117" href="#" class="fr-nav__link">Lien de navigation nav-6117</a>
                                        </li>
                                        <li>
                                            <a id="nav-6118" href="#" class="fr-nav__link">Lien de navigation nav-6118</a>
                                        </li>
                                        <li>
                                            <a id="nav-6119" href="#" class="fr-nav__link">Lien de navigation nav-6119</a>
                                        </li>
                                        <li>
                                            <a id="nav-6120" href="#" class="fr-nav__link">Lien de navigation nav-6120</a>
                                        </li>
                                    </ul>
                                </div>
                            </li>
                            <li class="fr-nav__item">
                                <button id="mega-menu-6121" aria-expanded="false" aria-controls="collapse-6122" type="mega-menu" class="fr-nav__btn">Entrée mega menu</button>
                                <div class="fr-collapse fr-mega-menu" id="collapse-6122">
                                    <div class="fr-container fr-container--fluid fr-container-lg">
                                        <div class="fr-grid-row fr-grid-row-lg--gutters">
                                            <div class="fr-col-12 fr-mb-n3v">
                                                <button aria-controls="collapse-6122" title="Fermer" type="button" id="button-6258" class="fr-btn--close fr-btn">Fermer</button>
                                            </div>
                                            <div class="fr-col-12 fr-col-lg-3">
                                                <h5 class="fr-mega-menu__category">
                                                    <a id="category-6123" href="#" class="fr-nav__link">Nom de catégorie</a>
                                                </h5>
                                                <ul class="fr-mega-menu__list">
                                                    <li>
                                                        <a id="nav-6124" href="#" class="fr-nav__link">Lien de navigation nav-6124</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6125" href="#" class="fr-nav__link">Lien de navigation nav-6125</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6126" href="#" class="fr-nav__link">Lien de navigation nav-6126</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6127" href="#" class="fr-nav__link">Lien de navigation nav-6127</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6128" href="#" class="fr-nav__link">Lien de navigation nav-6128</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6129" href="#" class="fr-nav__link">Lien de navigation nav-6129</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6130" href="#" class="fr-nav__link">Lien de navigation nav-6130</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6131" href="#" class="fr-nav__link">Lien de navigation nav-6131</a>
                                                    </li>
                                                </ul>
                                            </div>
                                            <div class="fr-col-12 fr-col-lg-3">
                                                <h5 class="fr-mega-menu__category">
                                                    <a id="category-6132" href="#" class="fr-nav__link">Nom de catégorie</a>
                                                </h5>
                                                <ul class="fr-mega-menu__list">
                                                    <li>
                                                        <a id="nav-6133" href="#" class="fr-nav__link">Lien de navigation nav-6133</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6134" href="#" class="fr-nav__link">Lien de navigation nav-6134</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6135" href="#" class="fr-nav__link">Lien de navigation nav-6135</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6136" href="#" class="fr-nav__link">Lien de navigation nav-6136</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6137" href="#" class="fr-nav__link">Lien de navigation nav-6137</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6138" href="#" class="fr-nav__link">Lien de navigation nav-6138</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6139" href="#" class="fr-nav__link">Lien de navigation nav-6139</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6140" href="#" class="fr-nav__link">Lien de navigation nav-6140</a>
                                                    </li>
                                                </ul>
                                            </div>
                                            <div class="fr-col-12 fr-col-lg-3">
                                                <h5 class="fr-mega-menu__category">
                                                    <a id="category-6141" href="#" class="fr-nav__link">Nom de catégorie</a>
                                                </h5>
                                                <ul class="fr-mega-menu__list">
                                                    <li>
                                                        <a id="nav-6142" href="#" class="fr-nav__link">Lien de navigation nav-6142</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6143" href="#" class="fr-nav__link">Lien de navigation nav-6143</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6144" aria-current="page" href="#" class="fr-nav__link">Lien de navigation nav-6144</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6145" href="#" class="fr-nav__link">Lien de navigation nav-6145</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6146" href="#" class="fr-nav__link">Lien de navigation nav-6146</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6147" href="#" class="fr-nav__link">Lien de navigation nav-6147</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6148" href="#" class="fr-nav__link">Lien de navigation nav-6148</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6149" href="#" class="fr-nav__link">Lien de navigation nav-6149</a>
                                                    </li>
                                                </ul>
                                            </div>
                                            <div class="fr-col-12 fr-col-lg-3">
                                                <h5 class="fr-mega-menu__category">
                                                    <a id="category-6150" href="#" class="fr-nav__link">Nom de catégorie</a>
                                                </h5>
                                                <ul class="fr-mega-menu__list">
                                                    <li>
                                                        <a id="nav-6151" href="#" class="fr-nav__link">Lien de navigation nav-6151</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6152" href="#" class="fr-nav__link">Lien de navigation nav-6152</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6153" href="#" class="fr-nav__link">Lien de navigation nav-6153</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6154" href="#" class="fr-nav__link">Lien de navigation nav-6154</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6155" href="#" class="fr-nav__link">Lien de navigation nav-6155</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6156" href="#" class="fr-nav__link">Lien de navigation nav-6156</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6157" href="#" class="fr-nav__link">Lien de navigation nav-6157</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6158" href="#" class="fr-nav__link">Lien de navigation nav-6158</a>
                                                    </li>
                                                </ul>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </li>
                            <li class="fr-nav__item">
                                <a id="nav-6159" type="link" href="#" class="fr-nav__link">accès direct nav-6159</a>
                            </li>
                            <li class="fr-nav__item">
                                <button id="menu-6160" aria-expanded="false" aria-controls="collapse-6161" aria-current="true" type="menu" class="fr-nav__btn">Entrée menu</button>
                                <div class="fr-collapse fr-menu" id="collapse-6161">
                                    <ul class="fr-menu__list">
                                        <li>
                                            <a id="nav-6162" href="#" class="fr-nav__link">Lien de navigation nav-6162</a>
                                        </li>
                                        <li>
                                            <a id="nav-6163" href="#" class="fr-nav__link">Lien de navigation nav-6163</a>
                                        </li>
                                        <li>
                                            <a id="nav-6164" aria-current="page" href="#" class="fr-nav__link">Lien de navigation nav-6164</a>
                                        </li>
                                        <li>
                                            <a id="nav-6165" href="#" class="fr-nav__link">Lien de navigation nav-6165</a>
                                        </li>
                                        <li>
                                            <a id="nav-6166" href="#" class="fr-nav__link">Lien de navigation nav-6166</a>
                                        </li>
                                        <li>
                                            <a id="nav-6167" href="#" class="fr-nav__link">Lien de navigation nav-6167</a>
                                        </li>
                                        <li>
                                            <a id="nav-6168" href="#" class="fr-nav__link">Lien de navigation nav-6168</a>
                                        </li>
                                        <li>
                                            <a id="nav-6169" href="#" class="fr-nav__link">Lien de navigation nav-6169</a>
                                        </li>
                                    </ul>
                                </div>
                            </li>
                            <li class="fr-nav__item">
                                <button id="mega-menu-6170" aria-expanded="false" aria-controls="collapse-6171" type="mega-menu" class="fr-nav__btn">Entrée mega menu</button>
                                <div class="fr-collapse fr-mega-menu" id="collapse-6171">
                                    <div class="fr-container fr-container--fluid fr-container-lg">
                                        <div class="fr-grid-row fr-grid-row-lg--gutters">
                                            <div class="fr-col-12 fr-mb-n3v">
                                                <button aria-controls="collapse-6171" title="Fermer" type="button" id="button-6259" class="fr-btn--close fr-btn">Fermer</button>
                                            </div>
                                            <div class="fr-col-12 fr-col-lg-8 fr-col-offset-lg-4--right">
                                                <div class="fr-mega-menu__leader">
                                                    <h4 class="fr-h4 fr-mb-2v">Titre éditorialisé</h4>
                                                    <p class="fr-hidden fr-unhidden-lg">Lorem ipsum dolor sit amet, consectetur adipiscing, incididunt, ut labore et dolore magna aliqua. Vitae sapien pellentesque habitant morbi tristique senectus et. Diam maecenas sed enim ut. Accumsan lacus vel facilisis volutpat est. Ut aliquam purus sit amet luctus. Lorem ipsum dolor sit amet consectetur adipiscing elit ut.</p>
                                                    <a id="link-6172" href="[url - à modifier]" class="fr-link fr-icon-arrow-right-line fr-link--icon-right">Voir toute la rubrique</a>
                                                </div>
                                            </div>
                                            <div class="fr-col-12 fr-col-lg-3">
                                                <h5 class="fr-mega-menu__category">
                                                    <a id="category-6173" href="#" class="fr-nav__link">Nom de catégorie</a>
                                                </h5>
                                                <ul class="fr-mega-menu__list">
                                                    <li>
                                                        <a id="nav-6174" href="#" class="fr-nav__link">Lien de navigation nav-6174</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6175" href="#" class="fr-nav__link">Lien de navigation nav-6175</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6176" href="#" class="fr-nav__link">Lien de navigation nav-6176</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6177" href="#" class="fr-nav__link">Lien de navigation nav-6177</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6178" href="#" class="fr-nav__link">Lien de navigation nav-6178</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6179" href="#" class="fr-nav__link">Lien de navigation nav-6179</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6180" href="#" class="fr-nav__link">Lien de navigation nav-6180</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6181" href="#" class="fr-nav__link">Lien de navigation nav-6181</a>
                                                    </li>
                                                </ul>
                                            </div>
                                            <div class="fr-col-12 fr-col-lg-3">
                                                <h5 class="fr-mega-menu__category">
                                                    <a id="category-6182" href="#" class="fr-nav__link">Nom de catégorie</a>
                                                </h5>
                                                <ul class="fr-mega-menu__list">
                                                    <li>
                                                        <a id="nav-6183" href="#" class="fr-nav__link">Lien de navigation nav-6183</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6184" href="#" class="fr-nav__link">Lien de navigation nav-6184</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6185" href="#" class="fr-nav__link">Lien de navigation nav-6185</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6186" href="#" class="fr-nav__link">Lien de navigation nav-6186</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6187" href="#" class="fr-nav__link">Lien de navigation nav-6187</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6188" href="#" class="fr-nav__link">Lien de navigation nav-6188</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6189" href="#" class="fr-nav__link">Lien de navigation nav-6189</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6190" href="#" class="fr-nav__link">Lien de navigation nav-6190</a>
                                                    </li>
                                                </ul>
                                            </div>
                                            <div class="fr-col-12 fr-col-lg-3">
                                                <h5 class="fr-mega-menu__category">
                                                    <a id="category-6191" href="#" class="fr-nav__link">Nom de catégorie</a>
                                                </h5>
                                                <ul class="fr-mega-menu__list">
                                                    <li>
                                                        <a id="nav-6192" href="#" class="fr-nav__link">Lien de navigation nav-6192</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6193" href="#" class="fr-nav__link">Lien de navigation nav-6193</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6194" aria-current="page" href="#" class="fr-nav__link">Lien de navigation nav-6194</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6195" href="#" class="fr-nav__link">Lien de navigation nav-6195</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6196" href="#" class="fr-nav__link">Lien de navigation nav-6196</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6197" href="#" class="fr-nav__link">Lien de navigation nav-6197</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6198" href="#" class="fr-nav__link">Lien de navigation nav-6198</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6199" href="#" class="fr-nav__link">Lien de navigation nav-6199</a>
                                                    </li>
                                                </ul>
                                            </div>
                                            <div class="fr-col-12 fr-col-lg-3">
                                                <h5 class="fr-mega-menu__category">
                                                    <a id="category-6200" href="#" class="fr-nav__link">Nom de catégorie</a>
                                                </h5>
                                                <ul class="fr-mega-menu__list">
                                                    <li>
                                                        <a id="nav-6201" href="#" class="fr-nav__link">Lien de navigation nav-6201</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6202" href="#" class="fr-nav__link">Lien de navigation nav-6202</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6203" href="#" class="fr-nav__link">Lien de navigation nav-6203</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6204" href="#" class="fr-nav__link">Lien de navigation nav-6204</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6205" href="#" class="fr-nav__link">Lien de navigation nav-6205</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6206" href="#" class="fr-nav__link">Lien de navigation nav-6206</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6207" href="#" class="fr-nav__link">Lien de navigation nav-6207</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6208" href="#" class="fr-nav__link">Lien de navigation nav-6208</a>
                                                    </li>
                                                </ul>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </li>
                            <li class="fr-nav__item">
                                <a id="nav-6209" type="link" href="#" class="fr-nav__link">accès direct nav-6209</a>
                            </li>
                            <li class="fr-nav__item">
                                <button id="mega-menu-6210" aria-expanded="false" aria-controls="collapse-6211" type="mega-menu" class="fr-nav__btn">Entrée mega menu</button>
                                <div class="fr-collapse fr-mega-menu" id="collapse-6211">
                                    <div class="fr-container fr-container--fluid fr-container-lg">
                                        <div class="fr-grid-row fr-grid-row-lg--gutters">
                                            <div class="fr-col-12 fr-mb-n3v">
                                                <button aria-controls="collapse-6211" title="Fermer" type="button" id="button-6260" class="fr-btn--close fr-btn">Fermer</button>
                                            </div>
                                            <div class="fr-col-12 fr-col-lg-3">
                                                <h5 class="fr-mega-menu__category">
                                                    <a id="category-6212" href="#" class="fr-nav__link">Nom de catégorie</a>
                                                </h5>
                                                <ul class="fr-mega-menu__list">
                                                    <li>
                                                        <a id="nav-6213" href="#" class="fr-nav__link">Lien de navigation nav-6213</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6214" href="#" class="fr-nav__link">Lien de navigation nav-6214</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6215" href="#" class="fr-nav__link">Lien de navigation nav-6215</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6216" href="#" class="fr-nav__link">Lien de navigation nav-6216</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6217" href="#" class="fr-nav__link">Lien de navigation nav-6217</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6218" href="#" class="fr-nav__link">Lien de navigation nav-6218</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6219" href="#" class="fr-nav__link">Lien de navigation nav-6219</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6220" href="#" class="fr-nav__link">Lien de navigation nav-6220</a>
                                                    </li>
                                                </ul>
                                            </div>
                                            <div class="fr-col-12 fr-col-lg-3">
                                                <h5 class="fr-mega-menu__category">
                                                    <a id="category-6221" href="#" class="fr-nav__link">Nom de catégorie</a>
                                                </h5>
                                                <ul class="fr-mega-menu__list">
                                                    <li>
                                                        <a id="nav-6222" href="#" class="fr-nav__link">Lien de navigation nav-6222</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6223" href="#" class="fr-nav__link">Lien de navigation nav-6223</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6224" href="#" class="fr-nav__link">Lien de navigation nav-6224</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6225" href="#" class="fr-nav__link">Lien de navigation nav-6225</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6226" href="#" class="fr-nav__link">Lien de navigation nav-6226</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6227" href="#" class="fr-nav__link">Lien de navigation nav-6227</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6228" href="#" class="fr-nav__link">Lien de navigation nav-6228</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6229" href="#" class="fr-nav__link">Lien de navigation nav-6229</a>
                                                    </li>
                                                </ul>
                                            </div>
                                            <div class="fr-col-12 fr-col-lg-3">
                                                <h5 class="fr-mega-menu__category">
                                                    <a id="category-6230" href="#" class="fr-nav__link">Nom de catégorie</a>
                                                </h5>
                                                <ul class="fr-mega-menu__list">
                                                    <li>
                                                        <a id="nav-6231" href="#" class="fr-nav__link">Lien de navigation nav-6231</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6232" href="#" class="fr-nav__link">Lien de navigation nav-6232</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6233" aria-current="page" href="#" class="fr-nav__link">Lien de navigation nav-6233</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6234" href="#" class="fr-nav__link">Lien de navigation nav-6234</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6235" href="#" class="fr-nav__link">Lien de navigation nav-6235</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6236" href="#" class="fr-nav__link">Lien de navigation nav-6236</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6237" href="#" class="fr-nav__link">Lien de navigation nav-6237</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6238" href="#" class="fr-nav__link">Lien de navigation nav-6238</a>
                                                    </li>
                                                </ul>
                                            </div>
                                            <div class="fr-col-12 fr-col-lg-3">
                                                <h5 class="fr-mega-menu__category">
                                                    <a id="category-6239" href="#" class="fr-nav__link">Nom de catégorie</a>
                                                </h5>
                                                <ul class="fr-mega-menu__list">
                                                    <li>
                                                        <a id="nav-6240" href="#" class="fr-nav__link">Lien de navigation nav-6240</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6241" href="#" class="fr-nav__link">Lien de navigation nav-6241</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6242" href="#" class="fr-nav__link">Lien de navigation nav-6242</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6243" href="#" class="fr-nav__link">Lien de navigation nav-6243</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6244" href="#" class="fr-nav__link">Lien de navigation nav-6244</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6245" href="#" class="fr-nav__link">Lien de navigation nav-6245</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6246" href="#" class="fr-nav__link">Lien de navigation nav-6246</a>
                                                    </li>
                                                    <li>
                                                        <a id="nav-6247" href="#" class="fr-nav__link">Lien de navigation nav-6247</a>
                                                    </li>
                                                </ul>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </li>
                            <li class="fr-nav__item">
                                <button id="menu-6248" aria-expanded="false" aria-controls="collapse-6249" type="menu" class="fr-nav__btn">Entrée menu</button>
                                <div class="fr-collapse fr-menu" id="collapse-6249">
                                    <ul class="fr-menu__list">
                                        <li>
                                            <a id="nav-6250" href="#" class="fr-nav__link">Lien de navigation nav-6250</a>
                                        </li>
                                        <li>
                                            <a id="nav-6251" href="#" class="fr-nav__link">Lien de navigation nav-6251</a>
                                        </li>
                                        <li>
                                            <a id="nav-6252" href="#" class="fr-nav__link">Lien de navigation nav-6252</a>
                                        </li>
                                        <li>
                                            <a id="nav-6253" href="#" class="fr-nav__link">Lien de navigation nav-6253</a>
                                        </li>
                                        <li>
                                            <a id="nav-6254" href="#" class="fr-nav__link">Lien de navigation nav-6254</a>
                                        </li>
                                        <li>
                                            <a id="nav-6255" href="#" class="fr-nav__link">Lien de navigation nav-6255</a>
                                        </li>
                                        <li>
                                            <a id="nav-6256" href="#" class="fr-nav__link">Lien de navigation nav-6256</a>
                                        </li>
                                        <li>
                                            <a id="nav-6257" href="#" class="fr-nav__link">Lien de navigation nav-6257</a>
                                        </li>
                                    </ul>
                                </div>
                            </li>
                        </ul>
                    </nav>
                </div>
            </div>
        </div>
        <div class="fr-mb-12v">
            <div class="fr-container">
                <section class="fr-accordion">
                    <h3 class="fr-accordion__title">
                        <button type="button" class="fr-accordion__btn" aria-expanded="false" aria-controls="snippet-6412">Extrait de code</button>
                    </h3>
                    <div id="snippet-6412" class="fr-collapse">
                        <pre class=" language-html"><code>&lt;nav class="fr-nav" id="navigation-6261" role="navigation" aria-label="Menu principal"&gt;
    &lt;ul class="fr-nav__list"&gt;
        &lt;li class="fr-nav__item"&gt;
            &lt;button id="menu-6262" aria-expanded="false" aria-controls="collapse-6263" type="menu" class="fr-nav__btn"&gt;Entrée menu&lt;/button&gt;
            &lt;div class="fr-collapse fr-menu" id="collapse-6263"&gt;
                &lt;ul class="fr-menu__list"&gt;
                    &lt;li&gt;
                        &lt;a id="nav-6264" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6264&lt;/a&gt;
                    &lt;/li&gt;
                    &lt;li&gt;
                        &lt;a id="nav-6265" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6265&lt;/a&gt;
                    &lt;/li&gt;
                    &lt;li&gt;
                        &lt;a id="nav-6266" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6266&lt;/a&gt;
                    &lt;/li&gt;
                    &lt;li&gt;
                        &lt;a id="nav-6267" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6267&lt;/a&gt;
                    &lt;/li&gt;
                    &lt;li&gt;
                        &lt;a id="nav-6268" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6268&lt;/a&gt;
                    &lt;/li&gt;
                    &lt;li&gt;
                        &lt;a id="nav-6269" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6269&lt;/a&gt;
                    &lt;/li&gt;
                    &lt;li&gt;
                        &lt;a id="nav-6270" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6270&lt;/a&gt;
                    &lt;/li&gt;
                    &lt;li&gt;
                        &lt;a id="nav-6271" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6271&lt;/a&gt;
                    &lt;/li&gt;
                &lt;/ul&gt;
            &lt;/div&gt;
        &lt;/li&gt;
        &lt;li class="fr-nav__item"&gt;
            &lt;button id="mega-menu-6272" aria-expanded="false" aria-controls="collapse-6273" type="mega-menu" class="fr-nav__btn"&gt;Entrée mega menu&lt;/button&gt;
            &lt;div class="fr-collapse fr-mega-menu" id="collapse-6273"&gt;
                &lt;div class="fr-container fr-container--fluid fr-container-lg"&gt;
                    &lt;div class="fr-grid-row fr-grid-row-lg--gutters"&gt;
                        &lt;div class="fr-col-12 fr-mb-n3v"&gt;
                            &lt;button aria-controls="collapse-6273" title="Fermer" type="button" id="button-6409" class="fr-btn--close fr-btn"&gt;Fermer&lt;/button&gt;
                        &lt;/div&gt;
                        &lt;div class="fr-col-12 fr-col-lg-3"&gt;
                            &lt;h5 class="fr-mega-menu__category"&gt;
                                &lt;a id="category-6274" href="#" class="fr-nav__link"&gt;Nom de catégorie&lt;/a&gt;
                            &lt;/h5&gt;
                            &lt;ul class="fr-mega-menu__list"&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6275" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6275&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6276" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6276&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6277" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6277&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6278" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6278&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6279" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6279&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6280" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6280&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6281" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6281&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6282" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6282&lt;/a&gt;
                                &lt;/li&gt;
                            &lt;/ul&gt;
                        &lt;/div&gt;
                        &lt;div class="fr-col-12 fr-col-lg-3"&gt;
                            &lt;h5 class="fr-mega-menu__category"&gt;
                                &lt;a id="category-6283" href="#" class="fr-nav__link"&gt;Nom de catégorie&lt;/a&gt;
                            &lt;/h5&gt;
                            &lt;ul class="fr-mega-menu__list"&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6284" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6284&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6285" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6285&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6286" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6286&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6287" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6287&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6288" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6288&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6289" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6289&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6290" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6290&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6291" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6291&lt;/a&gt;
                                &lt;/li&gt;
                            &lt;/ul&gt;
                        &lt;/div&gt;
                        &lt;div class="fr-col-12 fr-col-lg-3"&gt;
                            &lt;h5 class="fr-mega-menu__category"&gt;
                                &lt;a id="category-6292" href="#" class="fr-nav__link"&gt;Nom de catégorie&lt;/a&gt;
                            &lt;/h5&gt;
                            &lt;ul class="fr-mega-menu__list"&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6293" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6293&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6294" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6294&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6295" aria-current="page" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6295&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6296" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6296&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6297" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6297&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6298" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6298&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6299" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6299&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6300" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6300&lt;/a&gt;
                                &lt;/li&gt;
                            &lt;/ul&gt;
                        &lt;/div&gt;
                        &lt;div class="fr-col-12 fr-col-lg-3"&gt;
                            &lt;h5 class="fr-mega-menu__category"&gt;
                                &lt;a id="category-6301" href="#" class="fr-nav__link"&gt;Nom de catégorie&lt;/a&gt;
                            &lt;/h5&gt;
                            &lt;ul class="fr-mega-menu__list"&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6302" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6302&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6303" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6303&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6304" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6304&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6305" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6305&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6306" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6306&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6307" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6307&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6308" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6308&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6309" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6309&lt;/a&gt;
                                &lt;/li&gt;
                            &lt;/ul&gt;
                        &lt;/div&gt;
                    &lt;/div&gt;
                &lt;/div&gt;
            &lt;/div&gt;
        &lt;/li&gt;
        &lt;li class="fr-nav__item"&gt;
            &lt;a id="nav-6310" type="link" href="#" class="fr-nav__link"&gt;accès direct nav-6310&lt;/a&gt;
        &lt;/li&gt;
        &lt;li class="fr-nav__item"&gt;
            &lt;button id="menu-6311" aria-expanded="false" aria-controls="collapse-6312" aria-current="true" type="menu" class="fr-nav__btn"&gt;Entrée menu&lt;/button&gt;
            &lt;div class="fr-collapse fr-menu" id="collapse-6312"&gt;
                &lt;ul class="fr-menu__list"&gt;
                    &lt;li&gt;
                        &lt;a id="nav-6313" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6313&lt;/a&gt;
                    &lt;/li&gt;
                    &lt;li&gt;
                        &lt;a id="nav-6314" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6314&lt;/a&gt;
                    &lt;/li&gt;
                    &lt;li&gt;
                        &lt;a id="nav-6315" aria-current="page" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6315&lt;/a&gt;
                    &lt;/li&gt;
                    &lt;li&gt;
                        &lt;a id="nav-6316" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6316&lt;/a&gt;
                    &lt;/li&gt;
                    &lt;li&gt;
                        &lt;a id="nav-6317" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6317&lt;/a&gt;
                    &lt;/li&gt;
                    &lt;li&gt;
                        &lt;a id="nav-6318" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6318&lt;/a&gt;
                    &lt;/li&gt;
                    &lt;li&gt;
                        &lt;a id="nav-6319" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6319&lt;/a&gt;
                    &lt;/li&gt;
                    &lt;li&gt;
                        &lt;a id="nav-6320" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6320&lt;/a&gt;
                    &lt;/li&gt;
                &lt;/ul&gt;
            &lt;/div&gt;
        &lt;/li&gt;
        &lt;li class="fr-nav__item"&gt;
            &lt;button id="mega-menu-6321" aria-expanded="false" aria-controls="collapse-6322" type="mega-menu" class="fr-nav__btn"&gt;Entrée mega menu&lt;/button&gt;
            &lt;div class="fr-collapse fr-mega-menu" id="collapse-6322"&gt;
                &lt;div class="fr-container fr-container--fluid fr-container-lg"&gt;
                    &lt;div class="fr-grid-row fr-grid-row-lg--gutters"&gt;
                        &lt;div class="fr-col-12 fr-mb-n3v"&gt;
                            &lt;button aria-controls="collapse-6322" title="Fermer" type="button" id="button-6410" class="fr-btn--close fr-btn"&gt;Fermer&lt;/button&gt;
                        &lt;/div&gt;
                        &lt;div class="fr-col-12 fr-col-lg-8 fr-col-offset-lg-4--right"&gt;
                            &lt;div class="fr-mega-menu__leader"&gt;
                                &lt;h4 class="fr-h4 fr-mb-2v"&gt;Titre éditorialisé&lt;/h4&gt;
                                &lt;p class="fr-hidden fr-unhidden-lg"&gt;Lorem [...] elit ut.&lt;/p&gt;
                                &lt;a id="link-6323" href="[url - à modifier]" class="fr-link fr-icon-arrow-right-line fr-link--icon-right"&gt;Voir toute la rubrique&lt;/a&gt;
                            &lt;/div&gt;
                        &lt;/div&gt;
                        &lt;div class="fr-col-12 fr-col-lg-3"&gt;
                            &lt;h5 class="fr-mega-menu__category"&gt;
                                &lt;a id="category-6324" href="#" class="fr-nav__link"&gt;Nom de catégorie&lt;/a&gt;
                            &lt;/h5&gt;
                            &lt;ul class="fr-mega-menu__list"&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6325" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6325&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6326" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6326&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6327" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6327&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6328" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6328&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6329" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6329&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6330" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6330&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6331" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6331&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6332" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6332&lt;/a&gt;
                                &lt;/li&gt;
                            &lt;/ul&gt;
                        &lt;/div&gt;
                        &lt;div class="fr-col-12 fr-col-lg-3"&gt;
                            &lt;h5 class="fr-mega-menu__category"&gt;
                                &lt;a id="category-6333" href="#" class="fr-nav__link"&gt;Nom de catégorie&lt;/a&gt;
                            &lt;/h5&gt;
                            &lt;ul class="fr-mega-menu__list"&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6334" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6334&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6335" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6335&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6336" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6336&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6337" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6337&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6338" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6338&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6339" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6339&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6340" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6340&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6341" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6341&lt;/a&gt;
                                &lt;/li&gt;
                            &lt;/ul&gt;
                        &lt;/div&gt;
                        &lt;div class="fr-col-12 fr-col-lg-3"&gt;
                            &lt;h5 class="fr-mega-menu__category"&gt;
                                &lt;a id="category-6342" href="#" class="fr-nav__link"&gt;Nom de catégorie&lt;/a&gt;
                            &lt;/h5&gt;
                            &lt;ul class="fr-mega-menu__list"&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6343" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6343&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6344" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6344&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6345" aria-current="page" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6345&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6346" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6346&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6347" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6347&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6348" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6348&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6349" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6349&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6350" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6350&lt;/a&gt;
                                &lt;/li&gt;
                            &lt;/ul&gt;
                        &lt;/div&gt;
                        &lt;div class="fr-col-12 fr-col-lg-3"&gt;
                            &lt;h5 class="fr-mega-menu__category"&gt;
                                &lt;a id="category-6351" href="#" class="fr-nav__link"&gt;Nom de catégorie&lt;/a&gt;
                            &lt;/h5&gt;
                            &lt;ul class="fr-mega-menu__list"&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6352" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6352&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6353" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6353&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6354" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6354&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6355" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6355&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6356" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6356&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6357" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6357&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6358" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6358&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6359" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6359&lt;/a&gt;
                                &lt;/li&gt;
                            &lt;/ul&gt;
                        &lt;/div&gt;
                    &lt;/div&gt;
                &lt;/div&gt;
            &lt;/div&gt;
        &lt;/li&gt;
        &lt;li class="fr-nav__item"&gt;
            &lt;a id="nav-6360" type="link" href="#" class="fr-nav__link"&gt;accès direct nav-6360&lt;/a&gt;
        &lt;/li&gt;
        &lt;li class="fr-nav__item"&gt;
            &lt;button id="mega-menu-6361" aria-expanded="false" aria-controls="collapse-6362" type="mega-menu" class="fr-nav__btn"&gt;Entrée mega menu&lt;/button&gt;
            &lt;div class="fr-collapse fr-mega-menu" id="collapse-6362"&gt;
                &lt;div class="fr-container fr-container--fluid fr-container-lg"&gt;
                    &lt;div class="fr-grid-row fr-grid-row-lg--gutters"&gt;
                        &lt;div class="fr-col-12 fr-mb-n3v"&gt;
                            &lt;button aria-controls="collapse-6362" title="Fermer" type="button" id="button-6411" class="fr-btn--close fr-btn"&gt;Fermer&lt;/button&gt;
                        &lt;/div&gt;
                        &lt;div class="fr-col-12 fr-col-lg-3"&gt;
                            &lt;h5 class="fr-mega-menu__category"&gt;
                                &lt;a id="category-6363" href="#" class="fr-nav__link"&gt;Nom de catégorie&lt;/a&gt;
                            &lt;/h5&gt;
                            &lt;ul class="fr-mega-menu__list"&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6364" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6364&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6365" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6365&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6366" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6366&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6367" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6367&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6368" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6368&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6369" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6369&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6370" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6370&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6371" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6371&lt;/a&gt;
                                &lt;/li&gt;
                            &lt;/ul&gt;
                        &lt;/div&gt;
                        &lt;div class="fr-col-12 fr-col-lg-3"&gt;
                            &lt;h5 class="fr-mega-menu__category"&gt;
                                &lt;a id="category-6372" href="#" class="fr-nav__link"&gt;Nom de catégorie&lt;/a&gt;
                            &lt;/h5&gt;
                            &lt;ul class="fr-mega-menu__list"&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6373" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6373&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6374" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6374&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6375" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6375&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6376" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6376&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6377" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6377&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6378" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6378&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6379" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6379&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6380" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6380&lt;/a&gt;
                                &lt;/li&gt;
                            &lt;/ul&gt;
                        &lt;/div&gt;
                        &lt;div class="fr-col-12 fr-col-lg-3"&gt;
                            &lt;h5 class="fr-mega-menu__category"&gt;
                                &lt;a id="category-6381" href="#" class="fr-nav__link"&gt;Nom de catégorie&lt;/a&gt;
                            &lt;/h5&gt;
                            &lt;ul class="fr-mega-menu__list"&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6382" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6382&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6383" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6383&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6384" aria-current="page" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6384&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6385" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6385&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6386" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6386&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6387" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6387&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6388" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6388&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6389" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6389&lt;/a&gt;
                                &lt;/li&gt;
                            &lt;/ul&gt;
                        &lt;/div&gt;
                        &lt;div class="fr-col-12 fr-col-lg-3"&gt;
                            &lt;h5 class="fr-mega-menu__category"&gt;
                                &lt;a id="category-6390" href="#" class="fr-nav__link"&gt;Nom de catégorie&lt;/a&gt;
                            &lt;/h5&gt;
                            &lt;ul class="fr-mega-menu__list"&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6391" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6391&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6392" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6392&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6393" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6393&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6394" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6394&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6395" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6395&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6396" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6396&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6397" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6397&lt;/a&gt;
                                &lt;/li&gt;
                                &lt;li&gt;
                                    &lt;a id="nav-6398" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6398&lt;/a&gt;
                                &lt;/li&gt;
                            &lt;/ul&gt;
                        &lt;/div&gt;
                    &lt;/div&gt;
                &lt;/div&gt;
            &lt;/div&gt;
        &lt;/li&gt;
        &lt;li class="fr-nav__item"&gt;
            &lt;button id="menu-6399" aria-expanded="false" aria-controls="collapse-6400" type="menu" class="fr-nav__btn"&gt;Entrée menu&lt;/button&gt;
            &lt;div class="fr-collapse fr-menu" id="collapse-6400"&gt;
                &lt;ul class="fr-menu__list"&gt;
                    &lt;li&gt;
                        &lt;a id="nav-6401" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6401&lt;/a&gt;
                    &lt;/li&gt;
                    &lt;li&gt;
                        &lt;a id="nav-6402" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6402&lt;/a&gt;
                    &lt;/li&gt;
                    &lt;li&gt;
                        &lt;a id="nav-6403" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6403&lt;/a&gt;
                    &lt;/li&gt;
                    &lt;li&gt;
                        &lt;a id="nav-6404" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6404&lt;/a&gt;
                    &lt;/li&gt;
                    &lt;li&gt;
                        &lt;a id="nav-6405" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6405&lt;/a&gt;
                    &lt;/li&gt;
                    &lt;li&gt;
                        &lt;a id="nav-6406" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6406&lt;/a&gt;
                    &lt;/li&gt;
                    &lt;li&gt;
                        &lt;a id="nav-6407" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6407&lt;/a&gt;
                    &lt;/li&gt;
                    &lt;li&gt;
                        &lt;a id="nav-6408" href="#" class="fr-nav__link"&gt;Lien de navigation nav-6408&lt;/a&gt;
                    &lt;/li&gt;
                &lt;/ul&gt;
            &lt;/div&gt;
        &lt;/li&gt;
    &lt;/ul&gt;
&lt;/nav&gt;
</code></pre>
                    </div>
                </section>
            </div>
        </div>
    </div>
    <button aria-controls="fr-theme-modal" data-fr-opened="false" style="z-index: 1600" type="button" class="fr-display-button fr-background-default--grey fr-btn fr-btn--lg fr-icon-theme-fill fr-btn--secondary">Libellé bouton</button>
    <dialog id="fr-theme-modal" class="fr-modal" aria-labelledby="fr-theme-modal-title">
        <div class="fr-container fr-container--fluid fr-container-md">
            <div class="fr-grid-row fr-grid-row--center">
                <div class="fr-col-12 fr-col-md-6 fr-col-lg-4">
                    <div class="fr-modal__body">
                        <div class="fr-modal__header">
                            <button aria-controls="fr-theme-modal" title="Fermer" type="button" id="button-6413" class="fr-btn--close fr-btn">Fermer</button>
                        </div>
                        <div class="fr-modal__content">
                            <h1 id="fr-theme-modal-title" class="fr-modal__title">
                                Paramètres d’affichage
                            </h1>
                            <div id="fr-display" class="fr-display">
                                <fieldset class="fr-fieldset" id="display-fieldset">
                                    <legend class="fr-fieldset__legend--regular fr-fieldset__legend" id="display-fieldset-legend">
                                        Choisissez un thème pour personnaliser l’apparence du site.
                                    </legend>
                                    <div class="fr-fieldset__element">
                                        <div class="fr-radio-group fr-radio-rich">
                                            <input value="light" type="radio" id="fr-radios-theme-light" name="fr-radios-theme">
                                            <label class="fr-label" for="fr-radios-theme-light">
                                                Thème clair
                                            </label>
                                            <div class="fr-radio-rich__pictogram">
                                                <svg aria-hidden="true" class="fr-artwork" viewBox="0 0 80 80" width="80px" height="80px">
                                                    <use class="fr-artwork-decorative" href="../../../dist/artwork/pictograms/environment/sun.svg#artwork-decorative"></use>
                                                    <use class="fr-artwork-minor" href="../../../dist/artwork/pictograms/environment/sun.svg#artwork-minor"></use>
                                                    <use class="fr-artwork-major" href="../../../dist/artwork/pictograms/environment/sun.svg#artwork-major"></use>
                                                </svg>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="fr-fieldset__element">
                                        <div class="fr-radio-group fr-radio-rich">
                                            <input value="dark" type="radio" id="fr-radios-theme-dark" name="fr-radios-theme">
                                            <label class="fr-label" for="fr-radios-theme-dark">
                                                Thème sombre
                                            </label>
                                            <div class="fr-radio-rich__pictogram">
                                                <svg aria-hidden="true" class="fr-artwork" viewBox="0 0 80 80" width="80px" height="80px">
                                                    <use class="fr-artwork-decorative" href="../../../dist/artwork/pictograms/environment/moon.svg#artwork-decorative"></use>
                                                    <use class="fr-artwork-minor" href="../../../dist/artwork/pictograms/environment/moon.svg#artwork-minor"></use>
                                                    <use class="fr-artwork-major" href="../../../dist/artwork/pictograms/environment/moon.svg#artwork-major"></use>
                                                </svg>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="fr-fieldset__element">
                                        <div class="fr-radio-group fr-radio-rich">
                                            <input value="system" type="radio" id="fr-radios-theme-system" name="fr-radios-theme">
                                            <label class="fr-label" for="fr-radios-theme-system">
                                                Système
                                                <span class="fr-hint-text">Utilise les paramètres système</span>
                                            </label>
                                            <div class="fr-radio-rich__pictogram">
                                                <svg aria-hidden="true" class="fr-artwork" viewBox="0 0 80 80" width="80px" height="80px">
                                                    <use class="fr-artwork-decorative" href="../../../dist/artwork/pictograms/system/system.svg#artwork-decorative"></use>
                                                    <use class="fr-artwork-minor" href="../../../dist/artwork/pictograms/system/system.svg#artwork-minor"></use>
                                                    <use class="fr-artwork-major" href="../../../dist/artwork/pictograms/system/system.svg#artwork-major"></use>
                                                </svg>
                                            </div>
                                        </div>
                                    </div>
                                </fieldset>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </dialog>
    <script type="module" src="../../../dist/core/core.module.js"></script>
    <script type="module" src="../../../dist/component/navigation/navigation.module.js"></script>
    <script type="module" src="../../../dist/scheme/scheme.module.js"></script>
    <script type="module" src="../../../dist/component/accordion/accordion.module.js"></script>
    <script type="module" src="../../../dist/component/modal/modal.module.js"></script>
    <script type="module" src="../../../dist/component/display/display.module.js"></script>
    <script type="text/javascript" nomodule src="../../../dist/legacy/legacy.nomodule.js"></script>
    <script type="text/javascript" nomodule src="../../../dist/core/core.nomodule.js"></script>
    <script type="text/javascript" nomodule src="../../../dist/component/navigation/navigation.nomodule.js"></script>
    <script type="text/javascript" nomodule src="../../../dist/component/accordion/accordion.nomodule.js"></script>
    <script type="text/javascript" nomodule src="../../../dist/component/modal/modal.nomodule.js"></script>
    <script type="text/javascript" nomodule src="../../../dist/component/display/display.nomodule.js"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/prism/1.20.0/components/prism-core.min.js"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/prism/1.20.0/plugins/autoloader/prism-autoloader.min.js"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/prism/1.20.0/plugins/toolbar/prism-toolbar.min.js"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/prism/1.20.0/plugins/normalize-whitespace/prism-normalize-whitespace.min.js"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/prism/1.20.0/plugins/copy-to-clipboard/prism-copy-to-clipboard.min.js"></script>
</body>

</html>
